...
## 13. Mobile F1 Update — 9 June 2026

### Summary
BF ala carte multi-item booking added to mobile app. Flow matches web app exactly.
Dependencies updated and app.json schema fixed before build.

### Files Changed
- `mobile/src/screens/employee/BookMealScreen.js` — added multi-item ala carte selection with inline +/- quantity controls, dining mode selector, and single confirm button per session
- `mobile/src/services/messService.js` — added `bookAlaCarte` function (`POST /mess/reservations/alacarte`)
- `mobile/app.json` — moved `splash` from top-level property to `expo-splash-screen` plugin (SDK 56 schema fix)

### Dependencies Updated
- `expo` 56.0.3 → 56.0.9
- `expo-dev-client` 56.0.18 → 56.0.19
- `expo-font` installed (missing peer dependency for `@expo/vector-icons`)
- `expo-splash-screen` installed (required for new splash config)

### Design Decisions
- Ala carte section visible on Breakfast only — Lunch/Dinner show combo only (matches web)
- Items show inline +/- controls — quantity 0 means not selected
- Confirm panel appears only when at least one item has quantity > 0
- Dining mode selected once per session for all ala carte items (matches web)
- On confirm: all selected items submitted in one API call, selections reset to zero
- One combined notification fires per session (handled by backend — unchanged)
- Max quantity per item: 10 (matches backend validation)

### expo-doctor Result
21/21 checks passed. No issues detected.

### APK Build
EAS build triggered — preview profile — android platform.